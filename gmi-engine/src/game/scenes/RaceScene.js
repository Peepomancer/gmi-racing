...
export class RaceScene extends Phaser.Scene {
  ...
  update(time, delta) {
    try {
      ...
      if (this.isRacing && this.animationPlayer) {
        ...
        this.resolveAnimatedObstacleCollisions();
      }
  ...
  checkCrushedBalls(crusher) {
    ...
    try {
      ...
      if (targetWallPos === null) {
        switch (crusher.crusherDirection) {
          case 'right': targetWallPos = this.gameWidth; break;
          case 'left': targetWallPos = 0; break;
          case 'down': targetWallPos = this.gameHeight; break;
  ...
  resolveAnimatedObstacleCollisions() {
    if (!this.animationPlayer || this.animationPlayer.animations.size === 0) return;
    ...
    this.balls.forEach(ball => {
      ...
      this.obstacles.forEach(obs => {
        ...
        if (!obs.data?.id || !this.animationPlayer.hasAnimation(obs.data.id)) return;
        ...
        const crushedAgainstWall =
          (newX < margin && pushX < 0) ||
          (newX > this.gameWidth - margin && pushX > 0) ||
          (newY < margin && pushY < 0) ||
          (newY > this.gameHeight - margin && pushY > 0);
        ...
        newX = Math.max(margin, Math.min(this.gameWidth - margin, newX));
        newY = Math.max(margin, Math.min(this.gameHeight - margin, newY));
        ...
        if (newX >= this.gameWidth - margin) velX = -Math.abs(velX);
        if (newY <= margin) velY = Math.abs(velY);
        if (newY >= this.gameHeight - margin) velY = -Math.abs(velY);
